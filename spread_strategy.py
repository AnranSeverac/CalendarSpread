...
def generate_cheap_optionality_signals(
    spread_panel: pd.DataFrame,
    max_spread: float = 0.02,
    target_gap_days: float = 30.0,
    gap_tolerance_days: float = 5.0,
    min_tau_short_days: float = 5.0,
    exit_target_spread: float = 0.06,
) -> pd.DataFrame:
    """Strategy 'cheap_opt': BUY spreads where the implied window probability is ≤
    `max_spread` (default 2¢) AND the calendar gap (long_dd − short_dd) is one
    month ± `gap_tolerance_days` (default 25–35 days).

    Thesis: per-share max loss is exactly S (the spread itself); per-share max
    gain is (1 − S). At S = 0.02 that's a ~49:1 binary payoff if the event
    lands inside the window. We're not modeling the true window probability —
    just buying optionality whenever the market makes it cheap enough.

    `exit_target_spread` doubles as the "fair value" the downstream walker
    uses to compute edge against the bid-ask + fee cost. Setting it to 0.06
    means we want trades that have at least 4¢ of headroom from entry to
    target before the 2× edge/cost gate fires.

    Operates on the LATEST snapshot of the panel only — one signal per
    (event, short_dd, long_dd) at the most-recent timestamp.

    Output shape matches `generate_signals`: includes `direction='BUY'`,
    `mu=exit_target_spread`, `z=0`, `sigma=0`, `strategy='cheap_opt'`.
    """
    base_cols = ["direction", "mu", "z", "sigma", "strategy"]
    def _empty():
        out = spread_panel.iloc[:0].copy()
        for c, dt in zip(base_cols, [str, float, float, float, str]):
    ...
    p = spread_panel.copy()
    latest_ts = p["timestamp"].max()
    p = p[p["timestamp"] == latest_ts].copy()
    if "ladder_type" not in p.columns:
        p["ladder_type"] = "calendar"
    # `gap` is provided by build_spread_panel: days for calendar, threshold
    # distance for strike. Fall back to recomputing from dates for old panels.
    if "gap" not in p.columns:
        p["gap"] = (pd.to_datetime(p["long_dd"]) - pd.to_datetime(p["short_dd"])).dt.days.astype(float)

    common = (
        (p["spread"] <= max_spread)
        & (p["spread"] >= 0.0)
        & (p["tau_short"] >= min_tau_short_days)
    )
    is_cal = p["ladder_type"] == "calendar"
    # Calendar: require the one-month window. Strike: gap is a $-distance on a
    # totally different scale, so we don't apply the day-gap filter — any
    # adjacent-ish cheap band qualifies (cheapness + tau do the gating).
    cal_gap_ok = (
        (p["gap"] >= target_gap_days - gap_tolerance_days)
        & (p["gap"] <= target_gap_days + gap_tolerance_days)
    )
    cond = common & ((is_cal & cal_gap_ok) | (~is_cal))
    sub = p[cond].copy()
    if sub.empty:
        return _empty()

    sub["direction"] = "BUY"
    sub["mu"] = float(exit_target_spread)
    sub["z"] = 0.0
    sub["sigma"] = 0.0
    sub["strategy"] = "cheap_opt"
